...
curl --fail --silent --show-error --max-time 2 http://127.0.0.1:11434/api/tags
docker compose -f $HERMES_COMPOSE_FILE up -d hindsight
...
docker compose -f $HERMES_COMPOSE_FILE restart hindsight
curl --fail --silent --show-error --max-time 2 http://127.0.0.1:8888/health
...
@test "runs all eleven Unix acceptance phases in exact order and cleans up only after recovery" {
  ...
  recovery_line="$(grep -n "docker compose -f $HERMES_COMPOSE_FILE start hindsight" "$TEST_LOG" | cut -d: -f1)"
