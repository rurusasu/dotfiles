...
Describe 'CI workflow configuration' {
    ...
    It 'should run nixos-rebuild switch in a hosted WSL2 E2E workflow' {
        ...
        $script | Should -Match '(?s)systemctl start docker.*?"-d", \$DistroName, "-u", "nixos", "--".*?docker run --detach.*?docker inspect --format'
        $script | Should -Match 'CI_ASSERTION: seeded legacy Hermes gateway is running immediately before NixRebuildHandler\.Apply'
