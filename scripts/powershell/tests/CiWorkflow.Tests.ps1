...
Describe 'CI workflow configuration' {
    ...
    It 'should run nixos-rebuild switch in a hosted WSL2 E2E workflow' {
        ...
        $workflow | Should -Match 'check_required_job "WSL / Prebuild" "\$\{WSL_REQUIRED\}" "\$\{WSL_PREBUILD_RESULT\}"'
        ...
        $script | Should -Not -Match 'systemctl start docker|docker run --detach|docker inspect.*hermes|LegacyHermesGatewayStopped'
        ...
        $script | Should -Match '"bash", "-lc",\s+\$hermesVerifierCommand'
        $script | Should -Not -Match '"bash", "-lc",\s+@''\r?\nset -eu'
        $script | Should -Match 'Hermes CLI is missing from the NixOS user PATH'
        $script | Should -Match 'hermes_store_path="\$\(readlink -f "\$hermes_path"\)"'
