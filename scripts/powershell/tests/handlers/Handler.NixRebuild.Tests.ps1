...
Describe 'NixRebuildHandler' {
    ...
    Context 'Apply' {
        ...
        It 'should use exact running-container names instead of Docker name-filter patterns' {
            $ctx.Options['WithHermes'] = $true
            $script:wslArgs = ''
            Mock Invoke-Wsl {
                param($Arguments)
                $argStr = $Arguments -join ' '
                if ($argStr -match 'docker ps --format') { $script:wslArgs = $argStr }
                $global:LASTEXITCODE = 0
                return ''
            }
            ...
            $script:wslArgs | Should -Match 'docker ps --format.*grep -Fxq hermes'
            $script:wslArgs | Should -Match 'Docker container listing failed'
            $script:wslArgs | Should -Not -Match '--filter name='
        }
