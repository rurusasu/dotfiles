...
Describe 'Package catalog consistency' {
    Context 'Windows-only WSL package' {
        ...
        It 'should define Microsoft.WSL verification as a timed runtime check with winget repair recovery in the SSOT' {
            $sets = Get-Content -LiteralPath $script:setsPath -Raw

            $sets | Should -Match '(?s)"Microsoft\.WSL"\s*=\s*\{.*?command\s*=\s*"wsl".*?args\s*=\s*\[\s*"--version"\s*\].*?timeoutSeconds\s*=\s*30.*?recoveryStrategy\s*=\s*"wingetRepair"'
        }
        ...
        It 'should verify Microsoft.WSL by running wsl --version with timeout and repair recovery' {
            $json = Get-Content -LiteralPath $script:wingetJsonPath -Raw | ConvertFrom-Json
            $wingetSource = @($json.Sources | Where-Object { $_.SourceDetails.Name -eq 'winget' }) | Select-Object -First 1
            $package = @($wingetSource.Packages | Where-Object { $_.PackageIdentifier -eq 'Microsoft.WSL' }) | Select-Object -First 1
            ...
            $package.verifyCommand.recoveryStrategy | Should -Be 'wingetRepair'
        }
    }
}
