...
sandbox コンテナで Playwright CLI を動かすために以下の設定が必要:

- `capAdd: ["SYS_ADMIN"]` — Chromium の Linux sandbox（user namespace）に必要
- `shmSize: "256m"` — Chromium はデフォルトの `/dev/shm`（64MB）では不足
- `network: "bridge"` — 外部 URL へのアクセスに必要（`"none"` では不可）
- `PLAYWRIGHT_BROWSERS_PATH` 環境変数 — ビルド時にインストールしたブラウザのパスを明示

これらは `openclaw.docker.json.tmpl` の `sandbox.docker` セクションで設定済み。
