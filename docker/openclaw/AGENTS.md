...
Handler の `SeedCronJobs()` は **コンテナ内に `jobs.json` が存在しない場合のみ** シードをコピーする。
既に `jobs.json` がある場合（volume が残っている場合）はスキップされる。
...
| `workspace-git-sync`     | **10分ごと**（everyMs） | workspace を git add → commit → push → pull --ff-only  |
