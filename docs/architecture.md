...
対話端末で Python 3.9 以上が使える場合、見出しを同じ位置で `[RUNNING]` から
`[DONE]` / `[FAILED]` に更新します。説明と直近最大5行のログを dim 表示し、
成功時はログを折りたたんで完了見出しを残します。失敗時は直近ログも残します。
macOS パッケージ更新の失敗は `[ERROR] package: reason` として標準エラーにも出力し、
対話表示では最大20件の明示的なエラーを終了時に折り返して残します。
大きなアプリの取得に一律5分の制限は設けず、Nix の接続・無通信タイムアウトと
再試行設定に従います。低速でも転送が続く場合は時間がかかるため、必要なら Ctrl+C で中止できます。
詳細な通信エラーは下記の全出力ログを確認してください。
インストーラー全体を疑似端末で実行するため、シェル関数の状態と Nix・sudo などの
端末入力を維持します。画面では制御文字を整形しますが、全出力は権限 `0600` の
`dotfiles-install-*.log` に保存し、終了時（失敗を含む）に `Full log:` でパスを表示します。
保存先は一時ディレクトリ（`DOTFILES_INSTALL_LOG_DIR` で指定可能）です。
入力自体は記録しませんが、コマンドが出力する内容はログに含まれるため共有前に確認してください。
Python がない初期環境では従来の逐次表示を使用します。リダイレクト時、
`TERM=dumb`、`NO_COLOR` 指定時も逐次表示となり、装飾と追加ログ保存を無効にします。
