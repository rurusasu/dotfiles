...
`docs/hermes-agent/profile-home-layout.md` を参照する。install handler は runtime 側にも
`/opt/data/docs/profile-home-layout.md` を配置し、managed profile の `SOUL.md` から読めるようにする。
...
`researcher` profile を Slack から使う場合は、default gateway と別の Slack app/bot token を使う。
...
Hermes Agent の API / MCP 用 token も同じ `openclaw` vault から `~/.hermes/.env` に同期する。値は
`.env` にだけ書き、`config.yaml` には `${...}` 参照だけを置く。
