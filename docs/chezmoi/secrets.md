...
Slack メッセージを拒否する。
Hermes setup は `slack.require_mention: false` と `slack.allow_bots: mentions` を root と managed profile の `config.yaml` に設定する。
そのため Slack channel では明示的な bot mention がなくても、人間の通常メッセージを処理対象にする。
