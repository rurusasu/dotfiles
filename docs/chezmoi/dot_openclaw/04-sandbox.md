...
- Docker ソケットへのアクセスに root 権限が必要
- サンドボックス内でのツールインストールに root 権限が必要

### capAdd / shmSize の削除理由 (2026-03-09)

`capAdd: ["SYS_ADMIN"]` と `shmSize: "256m"` は OpenClaw `2026.3.x` のスキーマで認識されないキーとなり、コンテナ起動時にバリデーションエラー → 再起動ループを引き起こしていた。削除して解消。Playwright/Chromium の動作に影響がないか要経過観察。

### DOCKER_HOST の socat TCP プロキシ

サンドボックスコンテナから Docker ソケットに直接マウントする代わりに、`socat` による TCP プロキシ (`tcp://host.docker.internal:2375`) を経由する。これにより、サンドボックスコンテナからホスト上でシブリングコンテナを起動できる。

## セキュリティに関する注意事項

| リスク | 詳細 | 緩和策 |
|--------|------|--------|
| SYS_ADMIN ケーパビリティ | 広範な権限を持つケーパビリティであり、コンテナエスケープのリスクがある | readOnlyRoot + tmpfs + pidsLimit によるリスク軽減 |
| Docker ソケットアクセス | サンドボックスからホスト上のコンテナを作成・管理可能 | socat プロキシ経由に限定し、直接マウントを回避 |
