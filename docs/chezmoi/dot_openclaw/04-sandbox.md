...
| `workspaceAccess`        | `"none"`    | gateway workspace を sandbox に露出しない。**Windows + WSL + Docker 構成では `"rw"` にしてもパスマッピング不一致により gateway workspace への書き込みは反映されない**（sandbox は sibling コンテナで named volume を共有できない）。詳細は `docker/openclaw/AGENTS.md` の「Sandbox Workspace アクセスと永続化」セクション参照 |
