...
| `thread.initialHistoryLimit` | `50`                        | セッションリセット後にスレッド内メッセージを取得する上限 |

### Slack スレッドの文脈復元

`historyLimit` (セッション全体の履歴上限) とは異なり、`initialHistoryLimit` は新セッション開始時に Slack API からスレッド内メッセージを取得し、文脈を復元するために使用される。

> **注**: Slack の `retry` セクションは OpenClaw `2026.3.x` のスキーマで未サポート（Telegram では有効）。設定すると起動時にバリデーションエラーとなるため削除済み（2026-03-09）。

### Slack チャンネル設定

5 つのチャンネルが設定済み。すべて `requireMention: true` で動作する。
各チャンネルは `bindings` により専用エージェント（別プロセス）にルーティングされる。

| チャンネル ID | エージェント ID     |
| ------------- | ------------------- |
| `C0AK3SQKFV2` | `slack-C0AK3SQKFV2` |
| `C0AK70NLE6S` | `slack-C0AK70NLE6S` |
| `C0AK64UPVNW` | `slack-C0AK64UPVNW` |
| `C0AJVDKGN6A` | `slack-C0AJVDKGN6A` |
| `C0AJZSPV84S` | `slack-C0AJZSPV84S` |

各エージェントは独立したワークスペース (`/home/app/.openclaw/workspace-slack-<channelId>`) を持ち、セッション・メモリ・コンテキストが完全に分離される。

## 設計判断

### 1エージェント = 1チャンネル

- チャンネルごとに専用エージェントを割り当て、コンテキストの混入を防止
- `bindings` の `peer.kind: "channel"` で Slack チャンネル ID に基づきルーティング
- Telegram と Slack DM はマッチしないため `main` エージェントにフォールバック
