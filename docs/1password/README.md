...
5. 失敗しても apply や shell 起動を止めたくない箇所では、timeout と fallback を用意する。
...
- shell や GUI tool の secret は、起動時または deploy script 実行時に取得し、
...
- `op run --env-file`: shell / GUI launcher の子プロセスへ env をまとめて渡したい場合に使う。
