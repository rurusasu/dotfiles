...
Repository tooling remains under `scripts/validate_distribution.py`. Task 6 adds only the two named runtime scripts to root ownership, never the `scripts/` directory, so validator tooling is not copied into `/opt/data`. Own only `cron/jobs.json`, not `cron/`, so `/opt/data/cron/output` and `/opt/data/cron/state` remain mutable runtime data.
...
- Delete: `../hermes-home-distribution/skills/devops/hermes-home-backups/`
- Create: `../hermes-home-distribution/scripts/lifelog_sync.sh`
- Create: `../hermes-home-distribution/scripts/article_news_slack.sh`
- Modify: `../hermes-home-distribution/root-distribution.yaml`
- Modify: `../hermes-home-distribution/cron/jobs.json`
- Modify: `../hermes-home-distribution/SOUL.md`
- Modify: `../hermes-home-distribution/scripts/validate_distribution.py`
- Modify: `../hermes-home-distribution/tests/test_validate_distribution.py`

- [ ] Delete `scripts/hermes_home_sync.sh`. Add `scripts/lifelog_sync.sh` as a minimal `exec /usr/local/bin/hermes-bootstrap sync-repository lifelog` wrapper because Hermes cron only executes relative files beneath `HERMES_HOME/scripts`; the wrapper contains no Git or credential logic.

- [ ] Delete the obsolete `skills/devops/hermes-home-backups/` skill and reference. Runtime policy must never instruct an agent to initialize `/opt/data` as Git or recreate `hermes_home_sync.sh`.

- [ ] Port the final article-news Slack script currently generated by `Handler.HermesAgent.ps1` into `scripts/article_news_slack.sh`, preserving behavior while changing every lifelog content path to `/opt/data/shared/lifelog`.

- [ ] Replace the obsolete root-home sync cron entry with a default-profile-only lifelog sync job using `lifelog_sync.sh` at `20 4 * * *`. Preserve the existing lifelog job's `deliver` and `origin` metadata so operational output returns to its Slack thread. Add or preserve the article-news job using `article_news_slack.sh` at `0 */2 * * *`; it remains a no-agent job and posts through its own configured Slack channel, so do not add a second cron delivery target.

- [ ] Add only `scripts/article_news_slack.sh` and `scripts/lifelog_sync.sh` to `root-distribution.yaml`. Update the root validator so those exact regular files are allowed while `scripts/`, `scripts/validate_distribution.py`, traversal, symlinks, and special files remain rejected. Extend tests to prove validator tooling is not root-owned or copied into the production payload.
...
rg -n 'git (add|commit|fetch|pull|rebase|push)|GIT_ASKPASS' ../hermes-home-distribution/scripts/lifelog_sync.sh ../hermes-home-distribution/cron
test "$(sed -n '/^distribution_owned:/,$p' ../hermes-home-distribution/root-distribution.yaml | rg -c 'scripts/')" -eq 2
! sed -n '/^distribution_owned:/,$p' ../hermes-home-distribution/root-distribution.yaml | rg -n 'scripts/$|validate_distribution'
```
...
```bash
git -C ../hermes-home-distribution add root-distribution.yaml cron/jobs.json SOUL.md scripts/hermes_home_sync.sh scripts/lifelog_sync.sh scripts/article_news_slack.sh scripts/validate_distribution.py tests/test_validate_distribution.py skills/devops/hermes-home-backups
