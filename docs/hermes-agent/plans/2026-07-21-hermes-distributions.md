...
- [ ] Replace the obsolete root-home sync cron entry with a default-profile-only lifelog sync job using `lifelog_sync.sh`. Add or preserve the article-news Slack job using `article_news_slack.sh`; both must remain no-agent script jobs with their existing schedules and Slack delivery metadata.
