...
Before any target mutation, the installer verifies that `/usr/local` is a real directory, is owned by root, is not writable by group or other, has no extended ACL, and is not writable by the current caller. ACL presence is queried with macOS `find -acl` and normalized to an `absent|present` test seam; `ls -lde` display text is not parsed, and inspection failure or an unknown state fails closed. Normal `nrs` runs as the unprivileged login user; a root caller is writable and is intentionally rejected by this invariant. The installer then preflights both final components before the first mutation and repeats the parent and target checks immediately before each target sequence. Because `/usr` and the validated `/usr/local` parent are immutable to the invoking user, the user cannot replace either final directory entry between validation, `chown`, and `chmod`. Missing targets are created with non-recursive `mkdir --`; `mkdir -p` is not used.
