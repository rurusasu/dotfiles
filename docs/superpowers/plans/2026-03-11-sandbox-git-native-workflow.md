...
```jsonc
          "capDrop": ["ALL"],
          "binds": [
            "/app/data/cache/uv:/root/.cache/uv:rw",
            "/app/data/cache/pnpm:/root/.local/share/pnpm/store:rw",
            "/app/data/cache/bun:/root/.bun/install/cache:rw",
            "/app/data/cache/npm:/root/.npm:rw"
          ],
...
| ---------------------- | ------------------------------- | ------------------------------ |
| `/app/data/cache/uv`   | `/root/.cache/uv`               | uv パッケージキャッシュ        |
| `/app/data/cache/pnpm` | `/root/.local/share/pnpm/store` | pnpm content-addressable store |
| `/app/data/cache/bun`  | `/root/.bun/install/cache`      | bun パッケージキャッシュ       |
| `/app/data/cache/npm`  | `/root/.npm`                    | npm キャッシュ                 |
...
```bash
task sandbox:cache-init
docker exec openclaw ls -la /app/data/cache/
```

Expected: Directories `uv`, `pnpm`, `bun`, `npm` exist under `/app/data/cache/`.
